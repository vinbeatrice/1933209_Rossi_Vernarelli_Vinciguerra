...
The microservice utilizes the Java programming language, specifically targeting Java 17. The service is built using the Spring Boot framework (4.0.3) and the building is managed by Maven (3.9.9). 
Key components:
    - Spring-boot:
        - *spring-boot-starter-web* that is used for building web applications, including RESTful APIs;
    ...
    - CORS: it allows the presentation microservice do HTTP requests. 
...
The service is realized as follows:
    ...
    - SSE is used to receive stream data.
...
- ENDPOINTS:
    ...
    | GET | http://localhost:8081/discovery/rest_sensors/{sensor_id} | Returns the normalized version of the event retrieved from the Rest Sensor _sensor_id_. | 1 |
    ...
    | GET | http://localhost:8081/discovery/telemetry/mars/telemetry/{topic_id} | Returns the normalized stream of information coming from _topic_id_. | 2 |

## CONTAINER_NAME: Processing

### DESCRIPTION: 
This container handles CRUD operation on automation rules and the manual/automatic activation of actuators through the reception of explicit requests from the presentation service and the evaluation of automation rules on the events received from the ingestion service.
...
8. As a Mars operator, I want to be able to see the list of automation rules, so that I can manage them. (us #16)
...
- TYPE: backend
- DESCRIPTION:  
This microservice handles CRUD operation on automation rules and the manual/automatic activation of actuators through the reception of explicit requests from the presentation service and the evaluation of automation rules on the events received from the ingestion service.
- PORTS: 8082
- TECHNOLOGICAL SPECIFICATION:  
    The microservice utilizes the Java programming language, specifically targeting Java 17. The service is built using the Spring Boot framework (4.0.3) and the building is managed by Maven (3.9.9).
    Key components:
    - Spring-boot: 
        - *spring-boot-starter-web* that is used for building web applications, including RESTful APIs;
        - *spring-boot-starter-amqp* that provides a Spring Boot integration with AMPQ-based message brokers such as RabbitMQ;
        - *spring-boot-starter-data-jpa*: the service interacts with a relational database using JPA;
    - PostgreSQL: the service is configured to connect to a Postgres database;
    - CORS: it allows the presentation microservice do HTTP requests.
- SERVICE ARCHITECTURE: 
...
- TYPE: frontend
- DESCRIPTION: The Presentation container provides the user interface for the Mars monitoring system. It allows operators to visualize real-time sensor data, monitor actuator states and manage automation rules through a web-based dashboard. It also provides features for visualizing sensor trends, exporting charts as PDF and manually interacting with the system such as refreshing sensor values or toggling actuators and automation rules.
- PORTS: 8083
- TECHNOLOGICAL SPECIFICATION:  
The microservice utilizes the Java programming language, specifically targeting Java 17. The service is built using the Spring Boot framework (4.0.3) and the building is managed by Maven (3.9.9). 
Key components:
...
- SERVICE ARCHITECTURE:  
The service is realized with:
    - services that offers support for the live update of information;
    ...
    - a service that consumes events of automation rule triggering;
...
- PAGES:
    ...
    | actuators.html| list of all the actuators | processing | 9, 10|
